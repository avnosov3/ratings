...
class ImportDataService:
    ...
    async def _proccess_accommodation(
        ...
        accommodation_db = await self.accommodation_repository.create_accommodation(
    ...
    async def _proccess_file(
        ...
        file_db = await self.file_repository.create_file(
            file_in.model_dump(),
            session,
            commit=commit,
        )
    ...
    async def _proccess_amenity(
        ...
        if amenity not in self.amenity_mapper:
            ...
            amenity_db = await self.amenity_repository.create_amenity(
    ...
    async def _proccess_source(
        ...
        if source_in.name not in self.source_mapper:
            source = await self.source_repository.create_source(
    ...
    async def _proccess_locale(
        ...
        if locale_in.code not in self.locale_mapper:
            locale = await self.locale_repository.create_locale(
    ...
    async def _proccess_review(
        ...
        review_db = await self.review_repository.create_review(
